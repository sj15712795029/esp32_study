...
static void uart_gpio_reset(void)
{
    ...
    ESP_LOGI(tag, "HCI UART%d Pin select: TX 4, RX 18, CTS 23, RTS 19 Baudrate:%d", CONFIG_BTDM_CTRL_HCI_UART_NO, CONFIG_BTDM_CTRL_HCI_UART_BAUDRATE);

    uart_set_pin(CONFIG_BTDM_CTRL_HCI_UART_NO, 4, 18, 19, 23);
#endif
}
